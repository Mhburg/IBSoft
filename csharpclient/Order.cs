...
namespace IBApi
{
    ...
    public class Order : TWSLib.IOrder
    {
        ...
        */
        ...
        /*** For TagValue***/
        public string TVAlgoStrategyTag
        {
            get
            {
                return algoParams.Select(t => t.Tag).Aggregate((tagstring, next) => string.Concat(tagstring, ",", next));
            }
        }

        /*** For TagValue***/
        public string TVAlgoStrategeValue
        {
            get
            {
                return algoParams.Select(t => t.Value).Aggregate((valuestring, next) => string.Concat(valuestring, ",", next));
            }
        }

        /*** For TagValue***/
        public string TVSmartComboRoutingParamsTag
        {
            get
            {
                return smartComboRoutingParams.Select(t => t.Tag).Aggregate((tagstring, next) => string.Concat(tagstring, ",", next));
            }
        }

        /*** For TagValue***/
        public string TVSmartComboRoutingParamsValue
        {
            get
            {
                return smartComboRoutingParams.Select(t => t.Value).Aggregate((valuestring, next) => string.Concat(valuestring, ",", next));
            }
        }

        /*** For TagValue***/
        public string TVOrderMiscOptionsTag
        {
            get
            {
                return orderMiscOptions.Select(t => t.Tag).Aggregate((tagstring, next) => string.Concat(tagstring, ",", next));
            }
        }

        /*** For TagValue***/
        public string TVOrderMiscOptionsValue
        {
            get
            {
                return orderMiscOptions.Select(t => t.Value).Aggregate((valuestring, next) => string.Concat(valuestring, ",", next));
            }
        }

        /*** For OrderComboLeg***/
        public string OCLOrderComboLegs
        {
            get
            {
                return orderComboLegs.Select(t => t.Price.ToString()).Aggregate((pricestring, next) => string.Concat(pricestring, ",", next));
            }
        }
        #endregion
